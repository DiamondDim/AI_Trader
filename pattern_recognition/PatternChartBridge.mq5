...
int OnInit(){ EventSetTimer(MathMax(1,RefreshSeconds)); DrawPatterns(); return(INIT_SUCCEEDED); }
...
void ApplyCommon(string name,color clr,int width,ENUM_LINE_STYLE style){
...
void DrawLine(string parts[],int n,int seq){
  ...
  bool ray=(n>=10 && parts[9]=="true");
  if(ObjectCreate(0,name,OBJ_TREND,0,t1,p1,t2,p2)){
    ApplyCommon(name,clr,width,style);
...
void DrawHLine(string parts[],int n,int seq){
  if(n<4) return;
  string name=Prefix+parts[1]+"_"+IntegerToString(seq);
  double price=StringToDouble(parts[2]); color clr=(color)StringToInteger(parts[3]);
  int width=(n>=5?(int)StringToInteger(parts[4]):1);
  ENUM_LINE_STYLE style=(n>=6?ParseStyle(parts[5]):STYLE_SOLID);
  if(ObjectCreate(0,name,OBJ_HLINE,0,0,price)) ApplyCommon(name,clr,width,style);
}
...
void DrawText(string parts[],int n,int seq){
  ...
  string text=parts[4]; color clr=(color)StringToInteger(parts[5]); int size=(int)StringToInteger(parts[6]);
...
void DrawPatterns(){
  ...
  while(!FileIsEnding(h)){
    ...
    string parts[]; int n=StringSplit(line,'|',parts); if(n<=0) continue;
