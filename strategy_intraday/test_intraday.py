...
def _ensure_core_indicators(df: pd.DataFrame, include_ema_200: bool = True) -> pd.DataFrame:
    """Use canonical indicators while retaining legacy inline fallbacks."""
...
def run_intraday_backtest(symbol: str, timeframe_str: str, bars: int, initial_balance: float,
    ...
    """Universal intraday runner with cache-first market data."""
    ...
    try:
        ...
        df = provider.get_rates(symbol, timeframe_str, bars)
        if df.empty:
...
def _calculate_atr_inline(df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
    ...
    df['atr_14'] = ranges.max(axis=1).rolling(window=period).mean()
    return df
...
def run_mtf_backtest(symbol: str, main_timeframe: str, older_timeframe: str, bars_main: int, bars_older: int,
    ...
    try:
        ...
        if df_main.empty or df_older.empty:
            print("[!] Не удалось получить данные")
            return None
        df_main = _ensure_core_indicators(df_main, True)
        df_older = _ensure_core_indicators(df_older, True)
        # Keep the explicit cache API available for callers that relied on the
        # v2 runner; MarketDataProvider already uses the same DataCache backend.
        _ = cache
