...
def is_active_session(current_time) -> bool:
    """Проверяет, находимся ли мы в активной торговой сессии"""
...
def generate_fibonacci_pro_signals(df: pd.DataFrame, lookback: int = 30) -> List[Dict[str, Any]]:
    """
    Генерирует сигналы на основе уровней Фибоначчи.
    УЖЕСТОЧЁННАЯ ВЕРСИЯ с дополнительными фильтрами.

    Args:
        df: DataFrame с данными
        lookback: Период для поиска экстремумов (увеличен с 20 до 30)

    Returns:
        Список сигналов для бэктестера
    """
    signals = []
    df = _ensure_indicators(df)

    # Рассчитываем средний ATR за 50 баров для фильтра волатильности
    ...
    for i in range(lookback + 2, len(df)):
        ...
        # === ФИЛЬТР 1: Активная сессия ===
        if not is_active_session(current_time):
            continue

        # === ФИЛЬТР 2: Сила тренда (УЖЕСТОЧЕНО: ADX > 25 вместо 20) ===
        adx = current['adx_14']
        if adx < 25:
            continue

        # === ФИЛЬТР 3: Минимальная волатильность ===
        # Не торгуем, если текущий ATR < 50% от среднего (рынок "спит")
        ...
        # === ФИЛЬТР 4: Расстояние от EMA (не более 1%) ===
        distance_from_ema = abs(current['close'] - ema_50) / ema_50
        if distance_from_ema > 0.01:
            continue

        # === ШАГ 2: Находим swing high/low за lookback ===
        ...
        # === ШАГ 3: Рассчитываем уровни Фибоначчи ===
        fib_levels = _calculate_fibonacci_levels(swing_high, swing_low)
        current_price = current['close']

        # === ФИЛЬТР 5: Подтверждение силы свечи ===
        body_size = abs(current['close'] - current['open'])
        candle_range = current['high'] - current['low']
        if candle_range == 0 or (body_size / candle_range) < 0.5:
            continue  # Пропускаем свечи с маленьким телом

        # === ШАГ 4: Определяем направление тренда и ищем вход ===
        # === ЛОНГ ===
        if current_price > ema_50 and ema_50 > ema_200:
            # Восходящий тренд — ищем откат к уровням Фибо
            for fib_level_name, fib_price in fib_levels.items():
                # Входим на уровнях 0.236, 0.382, 0.5 (откат вверх)
                if fib_level_name in ['0.236', '0.382', '0.5']:
                    # УЖЕСТОЧЕНО: tolerance с 0.001 до 0.0005
                    if _is_bounce_above_level(df, i, fib_price, tolerance=0.0005):
                        # УЖЕСТОЧЕНО: Stochastic < 30 вместо < 40
        ...
        # === ШОРТ ===
            ...
            # Нисходящий тренд — ищем откат к уровням Фибо
            for fib_level_name, fib_price in fib_levels.items():
                # Входим на уровнях 0.618, 0.786, 0.764 (откат вниз)
                if fib_level_name in ['0.618', '0.786', '0.764']:
                    # УЖЕСТОЧЕНО: tolerance с 0.001 до 0.0005
                    if _is_bounce_below_level(df, i, fib_price, tolerance=0.0005):
                        # УЖЕСТОЧЕНО: Stochastic > 70 вместо > 60
                        stoch_k = current['stoch_k']
                        if stoch_k > 70:
                            signals.append({
                                'index': i,
...
def _find_swing_points(df: pd.DataFrame, current_idx: int, lookback: int) -> Tuple[float, float]:
    """
    Находит swing high и swing low за lookback период.
    Returns:
        (swing_high, swing_low) или (None, None) если не найдено
    """
    ...
    # Проверяем, что экстремумы не на краях (это настоящие swing points)
    high_idx = recent_highs.idxmax()
    low_idx = recent_lows.idxmin()
    current_idx_abs = df.index[current_idx]

    # Если экстремум на последней свече — это не swing point
...
def _calculate_fibonacci_levels(swing_high: float, swing_low: float) -> Dict[str, float]:
    """
    Рассчитывает уровни Фибоначчи для данного диапазона.
    Returns:
        Словарь {уровень: цена}
    """
    range_size = swing_high - swing_low
    return {
        '0.0': swing_low,
...
def _is_bounce_above_level(df: pd.DataFrame, current_idx: int, level: float, tolerance: float) -> bool:
    """
    Проверяет, был ли отскок цены вверх от уровня.
    Цена должна коснуться уровня (в пределах tolerance) и закрыться выше.
    """
    current = df.iloc[current_idx]
    prev = df.iloc[current_idx - 1]

    # Проверяем, что минимум свечи был near уровня
    min_price = min(current['low'], prev['low'])
    distance_to_level = abs(min_price - level) / level

    # Цена закрылась выше уровня
...
def _is_bounce_below_level(df: pd.DataFrame, current_idx: int, level: float, tolerance: float) -> bool:
    """
    Проверяет, был ли отскок цены вниз от уровня.
    Цена должна коснуться уровня (в пределах tolerance) и закрыться ниже.
    """
    current = df.iloc[current_idx]
    prev = df.iloc[current_idx - 1]

    # Проверяем, что максимум свечи был near уровня
